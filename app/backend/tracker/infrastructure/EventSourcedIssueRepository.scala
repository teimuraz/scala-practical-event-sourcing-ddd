...
import backend.tracker.api.event.{IssueEvent, MemberEvent}
...
class EventSourcedIssueRepository @Inject()
    (val db: Db, issueTopic: Topic[IssueEvent, RepComponents])
  extends IssueRepository with PgEventSourcedRepository[Issue, IssueId, IssueEvent] {

  override def topic: Option[Topic[IssueEvent, RepComponents]] = Some(issueTopic)
